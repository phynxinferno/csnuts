...
//    cout number of passes for slower mergesort
    cout << ceil(log2(length)) * 2 - 1 << endl;
        ...
        for(int i = 0; i < ceil(length / (double) period); i++) {
            ...
            for(int k = period * i + subperiod; k < period * (i+1); k++) {
                ...
                for(int j = topPosition; true; j--) {
                    ...
                    if(strcmp(arr[j+1].c_str(),arr[j].c_str()) == 1) swap(arr[j], arr[j+1]);
                    ...
                    else if(strcmp(arr[j+1].c_str(),arr[j].c_str()) != 0) {
                        bottomPosition = j + 2;
